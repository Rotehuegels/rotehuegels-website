...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    content.push({
      ...
      layout: { hLineWidth: (i: number, _n: any, node: any) => i === 0 || i === 1 || i === node.table.body.length ? 1.5 : 0.5, vLineWidth: () => 0.5, hLineColor: (i: number) => i <= 1 ? '#555' : '#ddd', vLineColor: () => '#ddd', paddingLeft: () => 4, paddingRight: () => 4, paddingTop: () => 3, paddingBottom: () => 3 },
      margin: [0, 0, 0, 8],
    });
