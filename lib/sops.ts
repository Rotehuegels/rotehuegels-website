...
const PRO_001: SOP = {
  ...
  procedure: [
    { step: 1, action: 'Add Stock Item', detail: 'Navigate to Procurement > Stock & Inventory. Use the inline add form to create new stock items. Enter: item name, item code, HSN code, unit of measure, and unit cost. On-hand quantity is derived from the stock_movements ledger — do not edit it directly.', system: '/d/stock > Add' },
    { step: 2, action: 'Record Stock Receipts', detail: 'Receipts post automatically when a GRN is created against a Purchase Order — see SOP-WH-001 for the full inward inspection flow. Each accepted line on a GRN writes a "receipt" event to the stock_movements ledger; the on-hand quantity is recomputed by trigger. Manual receipt entries are not supported (and not needed).', system: '/d/grn/new (per SOP-WH-001)' },
    { step: 3, action: 'Record Stock Issues', detail: 'Issues to operations / sales-order dispatch flow through Stock > Movements with movement_type=issue. Record: issue date, source linkage (order id / project), and reason. The ledger is the source of truth — never edit stock_items.quantity directly.', system: '/d/stock > Movements > New Issue' },
    { step: 4, action: 'Monitor Stock Levels', detail: 'Regularly review stock levels. Use the search function to find items by name, code, or HSN. Items at or below reorder_level are auto-flagged; the Auto-Indent cron raises a draft indent (per SOP-PRO-004) on a daily schedule for items below threshold.', system: '/d/stock' },
    { step: 5, action: 'Stock Valuation', detail: 'System computes valuation on a FIFO basis from the cost layers seeded by each receipt. Review the FIFO value and weighted-average cost on the stock detail page. Costs are not editable post-receipt — they reflect the actual invoice/PO price at the time of receipt.', system: '/d/stock/[id]' },
    { step: 6, action: 'Physical Verification', detail: 'Conduct periodic physical stock counts (at least quarterly). Compare physical counts with system records. Adjust discrepancies via Stock > Movements with movement_type=adjustment, with proper authorisation captured in the notes.', system: '/d/stock > Movements > Adjustment' },
    { step: 7, action: 'Deactivate Obsolete Stock', detail: 'Items no longer carried are deactivated (is_active=false) — they are hidden from new pickers but historical movements stay intact. Hard delete is only permitted when no movements have ever been posted; otherwise rely on deactivation. Approval required.', system: '/d/stock/[id] > Deactivate' },
  ],
  relatedDocs: ['SOP-ACC-004 (Purchase Order Processing)', 'SOP-PRO-004 (Indent Process)', 'SOP-WH-001 (Goods Receipt & Inspection)', 'SOP-PRO-002 (Shipment Tracking)', 'Stock Verification Sheet', 'Reorder Level Matrix'],
