...
export async function generateMetadata({ params }: { params: Promise<{ code: string }> }): Promise<Metadata> {
  ...
  const { data: r } = await supabaseAdmin
    .from('recyclers')
    .select('company_name, city, state, waste_type')
    .eq('recycler_code', code)
    .eq('is_active', true)
    .maybeSingle();
  if (!r) return { title: 'Recycler — Rotehügels' };
  const cat = CATEGORY_META[r.waste_type ?? ''];
  const title = `${r.company_name} — ${cat?.label ?? 'Recycler'} in ${r.state ?? 'India'} | Rotehügels`;
  return {
    title,
    description: `${r.company_name} · ${cat?.label ?? 'Recycler'} · ${r.city ? r.city + ', ' : ''}${r.state ?? ''}. Listed in the Rotehügels India Metals & Recycling Directory.`,
  };
}
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      <div className={`relative bg-gradient-to-br ${cat.tint} via-zinc-950 to-zinc-950`}>
        <div className="max-w-5xl mx-auto px-6 pt-10 pb-8">
          <Link href="/recycling/recyclers" className="inline-flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-300 mb-6">
          ...
          <div className={`${glass} overflow-hidden`}>
            ...
            <div className="divide-y divide-zinc-800/60">
              {related.map(o => {
                ...
                const Icon = m.icon;
                return (
                  <Link key={o.recycler_code} href={`/recycling/recyclers/${o.recycler_code}`}
