...
export default function QuotePrintButton() {
  const handlePrint = () => {
    const doc = document.getElementById('rh-quote-doc');
    if (!doc) return;

    // Clone the already-rendered document node
    const clone = doc.cloneNode(true) as HTMLElement;

    // Replace image srcs with data URLs (images already loaded in DOM — same-origin, no CORS issue)
    ...
    cloneImgs.forEach((cloneImg, i) => {
      ...
      if (orig?.complete && orig.naturalWidth > 0) {
        cloneImg.src = imgToDataUrl(orig);
      }
    });
