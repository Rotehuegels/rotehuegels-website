...
async function getSummary() {
  const [ordersRes, paymentsRes, expensesRes, stockRes] = await Promise.all([
    supabaseAdmin.from('orders').select('id, order_type, total_value_incl_gst, base_value, status, tds_deducted_total'),
    supabaseAdmin.from('order_payments').select('amount_received, tds_deducted, net_received'),
    supabaseAdmin.from('expenses').select('amount, expense_type'),
...
async function getRecentOrders() {
  const { data: orders } = await supabaseAdmin
    .from('orders')
    .select('id, order_no, order_type, client_name, total_value_incl_gst, status, order_date')
...
export default async function AccountsPage() {
  const [summary, recentOrders] = await Promise.all([getSummary(), getRecentOrders()]);
    ...
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">Financial overview — sales, receivables & expenses</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className={`${glass} p-6`}>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-zinc-500 uppercase tracking-wider">Order Book</p>
        ...
        <div className={`${glass} p-6`}>
          ...
          <p className="mt-1 text-xs text-zinc-600">Revenue received − expenses</p>
        </div>
      </div>
      ...
      <div className={`${glass} p-6`}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-zinc-300">Total Expenses</h2>
