...
export default function RecyclerList({ recyclers }: { recyclers: Recycler[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return recyclers.filter(r => {
      // Search
      if (q) {
        const hay = [r.company_name, r.facility_code, r.recycler_code, r.primary_category, r.city, r.state, r.email, r.phone, r.contact_person, r.address]
  ...
  function exportCsv() {
    const cols = [
      'facility_code', 'primary_category', 'recycler_code', 'company_name', 'waste_type',
      'city', 'state', 'pincode',
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Link
                          href={`/d/ecosystem/${r.recycler_code}`}
                          title={r.recycler_code ? `legacy ref: ${r.recycler_code}` : undefined}
                          className="text-xs font-mono text-amber-400 hover:text-amber-300 font-bold shrink-0"
                        >
                          {r.facility_code ?? r.recycler_code}
                        </Link>
