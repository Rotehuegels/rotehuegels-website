...
export default function EditQuotePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/accounts/items').then(r => r.json()),
      fetch(`/api/accounts/quotes/${id}`).then(r => r.json()),
    ]).then(([custRes, itemRes, quoteRes]) => {
      ...
      const cust = custs.find((c: Customer) => c.id === q.customer_id) ?? null;
      ...
      const isIntra = cust
        ? (cust.state_code === OUR_STATE_CODE || cust.state?.toLowerCase().includes('tamil'))
        : true;
      const existingItems = (q.items ?? []) as LineItem[];
      if (existingItems.length > 0) {
        setLines(existingItems.map(item => calcLine({
  ...
  function selectCustomer(custId: string) {
    const c = customers.find(c => c.id === custId) ?? null;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/accounts/quotes/${id}`, {
        method: 'PUT',
