...
// Pure types + constants live in lib/projectTasks.types.ts so client
// components can import them without pulling in the server-only bundle.
...
import type {
  ...
  TaskComment,
  ProjectLabel,
} from '@/lib/projectTasks.types';

// Re-export types + constants for server-side callers that prefer a
